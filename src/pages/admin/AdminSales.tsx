    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          {leaderboard.length > 0 && (
            <section>
              <h2 className="mb-3 flex items-center gap-2 font-display text-xl font-extrabold tracking-[-0.03em] text-white">
                <Trophy className="size-[1.1rem] text-warn-400" />
                Top reps
              </h2>
              <GlassCard className="divide-y divide-white/10">
                {leaderboard.map((r, i) => (
                  <div key={r.rep} className="flex items-center gap-3 px-4 py-3">
                    <span className="grid size-7 place-items-center rounded-full bg-warn-400/13 text-xs font-bold text-warn-100 ring-1 ring-warn-400/24 tnum">
                      {i + 1}
                    </span>
                    <span className="flex-1 font-mono text-sm font-semibold text-white">{r.rep}</span>
                    <span className="text-xs text-white/42 tnum">{r.count} sales</span>
                    <span className="w-24 text-right font-semibold text-money-200 tnum">
          ...
          <section>
            <h2 className="mb-3 font-display text-xl font-extrabold tracking-[-0.03em] text-white">Recent</h2>
              ...
              {sales.map((s) => (
                <motion.div key={s.id} variants={staggerItem}>
                  <GlassCard className="flex items-center gap-3 p-4">
                    ...
                    <span className="w-20 shrink-0 text-right font-bold text-white tnum">
    ...
    <GlassCard className="p-4">
      ...
      <p
        className={`mt-1 font-display text-2xl font-extrabold tracking-[-0.02em] tnum ${
          accent ? 'text-money-200' : 'text-white'
        }`}
      >
        {value}
      </p>
